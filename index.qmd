...
``` python
!conda install -c conda-forge numpy pandas geopandas matplotlib seaborn python-dotenv openpyxl libpysal
...
```{python}
# Prepare working dataset - Only keep direct ACS reported median
df_bg_income_process = gdf_bg_income[[
    'GEOID', 'COUNTYFP', 'NAMELSAD', 'HH_TOTAL', 'HH_MED_INC'
]].copy()

# Define validity conditions for ACS reported median
# Valid if: Not NaN, > 0, and not an extreme outlier (>= $250k)
valid_condition = (
    (df_bg_income_process['HH_MED_INC'].notna()) &
    (df_bg_income_process['HH_MED_INC'] > 0) &
    (df_bg_income_process['HH_MED_INC'] < 250001)
)

# Set strictly to NaN if invalid
df_bg_income_process['Median Income'] = np.where(
    valid_condition,
    df_bg_income_process['HH_MED_INC'],
    np.nan
)

# Add GEOID_BG column for joining later
df_bg_income_process['GEOID_BG'] = df_bg_income_process['GEOID']

print(f"Total Block Groups: {len(df_bg_income_process)}")
print(f"Valid ACS Medians: {df_bg_income_process['Median Income'].notna().sum()}")
print(f"Missing/Invalid Medians (to be spatially filled): {df_bg_income_process['Median Income'].isna().sum()}")
...
gdf_bg_with_income = gdf_bg_bound[['GEOID', 'geometry']].merge(
    df_bg_income_process[['GEOID_BG', "NAMELSAD", "HH_TOTAL", "Median Income"]],
    left_on='GEOID',
    right_on='GEOID_BG',
    how='inner'
).to_crs(PROJECT_CRS)
...
```{python}
# Join strict Median Income (with NaNs) from Block Groups to Blocks
...
# --- SPATIAL SMOOTHING START ---
print("Starting Spatial Smoothing for missing block incomes...")
print(f"Initial missing blocks: {gdf_block_hh_income['Median Income'].isna().sum()}")
```

```{python}
# Create spatial weights matrix (Queen contiguity)
# We use the dataframe index to identify neighbors
w = Queen.from_dataframe(gdf_block_hh_income, use_index=True)

# Iterative filling loop
max_iterations = 5
for i in range(max_iterations):
    # Identify rows that are still missing income
    missing_indices = gdf_block_hh_income[gdf_block_hh_income['Median Income'].isna()].index

    # Dictionary to store updates for this iteration
    updates = {}

    for idx in missing_indices:
        # Get neighbors for this block
        if idx in w.neighbors:
            neighbor_indices = w.neighbors[idx]

            # Extract income values of these neighbors
            neighbor_incomes = gdf_block_hh_income.loc[neighbor_indices, 'Median Income']

            # Calculate mean of VALID neighbors only (ignoring NaNs)
            valid_neighbors = neighbor_incomes.dropna()

            if not valid_neighbors.empty:
                updates[idx] = valid_neighbors.mean()

    # Apply updates from this iteration
    if updates:
        # We use fillna with a series mapped by index to update specific rows
        update_series = pd.Series(updates)
        gdf_block_hh_income['Median Income'] = gdf_block_hh_income['Median Income'].fillna(update_series)
        print(f"Iteration {i+1}: Filled {len(updates)} blocks. Remaining missing: {gdf_block_hh_income['Median Income'].isna().sum()}")
    else:
        print(f"Iteration {i+1}: No new blocks could be filled. Stopping early.")
        break

print("Spatial Smoothing Complete.")
# --- SPATIAL SMOOTHING END ---
